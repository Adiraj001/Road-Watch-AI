...
# ══════════════════════════════════════════════════════════════════════
# ZONE REMAPPING — translate DB zone names → display names
# Change any entry here to rename zones across the entire dashboard
...
def _remap_zone(name: str) -> str:
    """Return the display name for a zone, falling back to the original."""
    return ZONE_REMAP.get(name, name)


def _remap_zone_list(zones: list[dict]) -> list[dict]:
    """Apply ZONE_REMAP to a list of zone dicts that contain a 'zone' key."""
    remapped = []
    for z in zones:
        z2 = dict(z)
        z2["zone"] = _remap_zone(z2.get("zone", ""))
        remapped.append(z2)
    return remapped


def _remap_records(records: list[dict]) -> list[dict]:
    """Apply ZONE_REMAP to a list of pothole records."""
    remapped = []
    for r in records:
        r2 = dict(r)
        r2["zone"] = _remap_zone(r2.get("zone", ""))
        remapped.append(r2)
    return remapped
...
def _data_potholes(limit: int = 100) -> list:
    ...
    try:
        records = get_all_potholes(limit=limit)
        return _remap_records(records)
...
def _data_stats() -> dict:
    ...
    try:
        ...
        zones = _remap_zone_list(get_zone_counts())
        return {
            **counts,
            "fix_rate": fix_rate,
...
def _register_callbacks(app) -> None:
    ...
    def update_charts(_):
        s = _data_stats()
        records = _data_potholes(limit=10000)  # already remapped
        ...
        # ── Zone horizontal bar  (already remapped via _data_stats)
        zones = s.get("zones", [])
        zone_fig = go.Figure(
            go.Bar(
        ...
        # ── Zone progress bars  (records already remapped, zones already remapped)
        progress = []
        for z in zones[:8]:
            name = z["zone"]   # already the remapped display name
            total = z["count"]
            fixed_n = sum(
                1 for r in records
    ...
    def update_hotspots(_):
        hotspots = _data_hotspots()   # already remapped
        if not hotspots:
            return html.Div(
    ...
    def update_table(_, status_filter):
        records = _data_potholes(limit=50)   # already remapped
